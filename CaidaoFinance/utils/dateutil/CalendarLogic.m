...
{
    static CalendarLogic * sharedAccountManagerInstance = nil;
...
{
    NSMutableArray*allMonths = [NSMutableArray new];//有假数据
    NSMutableArray*cleanAllMonth = [NSMutableArray new];//干净的数据
    NSDate*date = [NSDate date];
    NSMutableArray*daysCount = [date numberOfDaysInMonthAtYear:year];
    NSInteger monthCount = 12;
    NSDictionary * remind = [GZDataUtils applicationDictionaryFromFile:@"Remind.plist"];
    NSArray * reminds = [remind allValues];
    //放入一整年的数据 并转换成CalendarDayModel
    for (int i = 1; i<=monthCount; i++) {
        NSMutableArray*month = [NSMutableArray new];
        NSMutableArray*cleanMonth = [NSMutableArray new];
        NSInteger count = [daysCount[i-1] integerValue];
        NSDate*currentMonthDate = [NSDate dateByMonthAndYear:year month:i];
        NSInteger gap = [NSDate weeklyOrdinality:currentMonthDate];
        
        for (int k = 0; k < gap - 1; k++) {
            CalendarDayModel*modelgap = [CalendarDayModel calendarDayWithYear:year month:i day:1];
            modelgap.style = CellDayTypeBlank;
            [month addObject:modelgap];
        }
        for (int j = 1; j <= count; j++) {
            CalendarDayModel*model = [CalendarDayModel calendarDayWithYear:year month:i day:j];
            model.week = [[model date]getWeekIntValueWithDate];
            [self changStyle:model];
            [month addObject:model];
            [cleanMonth addObject:model];
        }
        
        [allMonths addObject:month];
        [cleanAllMonth addObject:cleanMonth];
    }
    
    //放入本地提醒数据
    for (NSDictionary * dic in reminds) {
        NSDate * date = [[NSDate new] dateFromStringTypeTwo:[dic objectForKey:@"oldTime"]];
        NSDateComponents * dateCompon = [date YMDComponents];
        NSInteger dyear = dateCompon.year;
        NSInteger dmonth = dateCompon.month;
        NSInteger dday = dateCompon.day;
        if (dyear == year) {
            NSArray * monthArr = allMonths[dmonth - 1];
            for (CalendarDayModel * model in monthArr) {
                if (dday == model.day && model.style != CellDayTypeBlank) {
                    model.isRemind = YES;
                }
            }
            NSArray * monthCleanArr = cleanAllMonth[dmonth - 1];
            for (CalendarDayModel * model in monthCleanArr) {
                if (dday == model.day && model.style != CellDayTypeBlank) {
                    model.isRemind = YES;
                }
            }
    ...
    if(year == [[NSDate date]YMDComponents].year)GZAPP.years = cleanAllMonth;
    else if (year > [[NSDate date]YMDComponents].year)GZAPP.nextYears = cleanAllMonth;
...
- (void)changStyle:(CalendarDayModel *)calendarDay
{
    ...
    NSDateComponents *calendarToDay  = [[NSDate date] YMDComponents];//今天
    NSDateComponents *calendarbefore = [before YMDComponents];//最后一天
//    NSDateComponents *calendarSelect = [select YMDComponents];//选择的那一天
