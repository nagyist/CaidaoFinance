...
@interface GZMenuView ()<UITableViewDataSource,UITableViewDelegate>
{
...
+ (void)initialize
{
    if (self != [GZMenuView class])
        return;
    GZMenuView*appearance = [self appearance];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    GZMenuItem*item = [[GZMenuItem alloc] initWithText:[_dataSource stringFromIndex:indexPath.row view:self]];
...
+(void)initialize
{
    if (self != [GZMenuItem class]) {
        return;
    }
    GZMenuItem*item = [self appearance];
