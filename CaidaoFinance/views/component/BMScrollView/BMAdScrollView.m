...
//             make.height.mas_equalTo(self.mas_height);
        ...
        if (titleFrame.size.height == 0.0) {
            titleFrame = CGRectMake(0, self.frame.size.height - 40, 320, 30);
        }
...
//    [imageSV mas_makeConstraints:^(MASConstraintMaker*make)
//     {
//         make.top.equalTo(self.mas_top).with.offset(0); //with is an optional semantic filler
...
//    }
    ...
    for (int i = 0; i <= imageArr.count +1; i++) {
        ...
        //创建内容对象
        CGRect titleRect = CGRectMake(0, 120, 320, 30);
//        BMImageView*lastView;
        BMImageView *imageView =  [[BMImageView alloc]initWithImageName:imageURL title:title x:WIDTH*i tFrame:titleRect iHeight:imageSV.frame.size.height titleHidden:NO];
...
//             make.left.equalTo(@0);
//             make.width.equalTo(imageSV.mas_width);
//             make.height.equalTo(imageSV.mas_height);
//         }];
//        lastView = imageView;
    }
...
{
    CGRect rect =  CGRectMake(0, 140, WIDTH, PAGE_HEIGHT);
...
//    pageControl.currentPageIndicatorTintColor = RGBCOLOR(239, 115, 58);
