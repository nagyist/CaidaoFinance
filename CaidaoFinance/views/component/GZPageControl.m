...
@interface GZPageControl ()
@end

@implementation GZPageControl
{
@private
...
- (void)_initialize
{
	...
	_alignment = GZPageControlAlignmentCenter;
	_verticalAlignment = GZPageControlVerticalAlignmentMiddle;
...
//        AtPoint:CGPointMake(xOffset, yOffset)
		if (image) {
...
- (CGFloat)_leftOffset
{
	...
	CGFloat left = 0.0f;
	switch (_alignment) {
		case GZPageControlAlignmentCenter:
			left = CGRectGetMidX(rect) - (size.width / 2.0f);
			break;
		case GZPageControlAlignmentRight:
			left = CGRectGetMaxX(rect) - size.width - 30;
			break;
		default:
			break;
	}
...
- (CGFloat)_topOffset
{
	...
	switch (_verticalAlignment) {
		case GZPageControlVerticalAlignmentMiddle:
			top = CGRectGetMidY(rect) - (_measuredIndicatorHeight / 2.0f);
			break;
		case GZPageControlVerticalAlignmentBottom:
