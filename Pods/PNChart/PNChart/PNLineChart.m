...
@property (nonatomic) NSMutableArray *chartVerticalArray;
...
@property (nonatomic) NSMutableArray *verticalPath;
...
- (void)strokeChart
{
    ...
    _verticalPath = [[NSMutableArray alloc] init];
    
    [self calculateChartPath:_chartPath andPointsPath:_pointPath andPathKeyPoints:_pathPoints andPathStartEndPoints:_endPointsOfPath verticalPath:_verticalPath];
    // Draw each line
    for (NSUInteger lineIndex = 0; lineIndex < self.chartData.count; lineIndex++) {
        ...
        CAShapeLayer *verticalLine = (CAShapeLayer *)self.chartVerticalArray[lineIndex];
        ...
        UIBezierPath *verticalPath = [_verticalPath objectAtIndex:lineIndex];
        ...
        [verticalLine addAnimation:pathAnimation forKey:@"strokeEndAnimation"];
...
- (void)pointAction:(UIButton*)sender
{
    ...
    PNLineChartData*data = [self.chartData firstObject];
    NSString * text = [NSString stringWithFormat:@"%@",[data.chartDatas objectAtIndex:[sender tag]]];
    CGFloat y = sender.center.y - 20;
    self.tipView.center = CGPointMake(sender.center.x, y);
    self.tipLabel.text = text;
}

- (void)calculateChartPath:(NSMutableArray *)chartPath andPointsPath:(NSMutableArray *)pointsPath andPathKeyPoints:(NSMutableArray *)pathPoints andPathStartEndPoints:(NSMutableArray *)pointsOfPath verticalPath:(NSMutableArray*)verticalPath
{
    ...
    for (NSUInteger lineIndex = 0; lineIndex < self.chartData.count; lineIndex++) {
        ...
        [verticalPath insertObject:verticalline atIndex:lineIndex];
...
#warning modify chart path
            ...
            if (chartData.inflexionPointStyle == PNLineChartPointStyleCircle) {
                ...
                bt.tag = i;
                [bt addTarget:self
                       action:@selector(pointAction:)
             forControlEvents:UIControlEventTouchUpInside];
                
                [self addSubview:bt];
                
                [verticalline moveToPoint:CGPointMake(x, y + inflexionWidth/2)];
...
- (void)setChartData:(NSArray *)data
{
    if (data != _chartData) {
        ...
        self.chartVerticalArray= [NSMutableArray arrayWithCapacity:data.count];
        ...
        for (PNLineChartData *chartData in data) {
            ...
            pointLayer.lineWidth     = chartData.circleLineWidth;
            [self.layer addSublayer:pointLayer];
            [self.chartPointArray addObject:pointLayer];
            
            CAShapeLayer *verticalLayer = [CAShapeLayer layer];
            verticalLayer.strokeColor   = [[chartData.color colorWithAlphaComponent:chartData.alpha]CGColor];
            verticalLayer.lineCap       = kCALineCapButt;
            verticalLayer.lineJoin      = kCALineJoinMiter;
            verticalLayer.fillColor     = [[UIColor whiteColor] CGColor];
            verticalLayer.lineWidth     = chartData.lineWidth;
            verticalLayer.strokeEnd     = 0.0;
            [self.layer addSublayer:verticalLayer];
            [self.chartVerticalArray addObject:verticalLayer];
        }
...
- (void)updateChartData:(NSArray *)data
{
    ...
    for (NSUInteger lineIndex = 0; lineIndex < self.chartData.count; lineIndex++) {
        ...
        CAShapeLayer *chartLine = (CAShapeLayer *)self.chartLineArray[lineIndex];
        CAShapeLayer *pointLayer = (CAShapeLayer *)self.chartPointArray[lineIndex];
        CAShapeLayer *verticalLayer = (CAShapeLayer *)self.chartVerticalArray[lineIndex];

        
        UIBezierPath *progressline = [_chartPath objectAtIndex:lineIndex];
        UIBezierPath *verticalline = [_chartVerticalArray objectAtIndex:lineIndex];
        UIBezierPath *pointPath = [_pointPath objectAtIndex:lineIndex];
        ...
        CABasicAnimation * verticalAnimation = [CABasicAnimation animationWithKeyPath:@"path"];
        verticalAnimation.fromValue = (id)chartLine.path;
        verticalAnimation.toValue = (id)[progressline CGPath];
        verticalAnimation.duration = 0.5f;
        verticalAnimation.autoreverses = NO;
        verticalAnimation.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut];
        [verticalLayer addAnimation:verticalAnimation forKey:@"animationKey"];
        ...
        chartLine.path = progressline.CGPath;
        pointLayer.path = pointPath.CGPath;
        verticalLayer.path = verticalline.CGPath;
...
- (void)setupTipView
{
    self.tipView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 65, 29)];
    self.tipView.backgroundColor = [UIColor clearColor];
    UIImageView*bg = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 65, 29)];
    [bg setImage:[UIImage imageNamed:@"individual_chart_alert_bg"]];
    [self.tipView addSubview:bg];
    self.tipLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 65, 10)];
    self.tipLabel.textColor = [UIColor colorWithRed:139.0/255.0 green:32.0/255.0 blue:35.0/255.0 alpha:1.0];
    self.tipLabel.font = [UIFont systemFontOfSize:12];
    self.tipLabel.textAlignment = NSTextAlignmentCenter;
    self.tipLabel.center = self.tipView.center;
