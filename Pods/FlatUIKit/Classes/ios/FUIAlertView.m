...
- (id)initWithTitle:(NSString *)title
  ...
  otherButtonTitles:(NSString *)otherButtonTitles, ... {
    ...
    if (self) {
        ...
        [backgroundOverlay addGestureRecognizer:gesture];
...
{
    [self dismissWithClickedButtonIndex:999 animated:YES];
}
...
- (void) layoutSubviews {
    ...
    if (CGAffineTransformIsIdentity(self.alertContainer.transform)) {
        ...
        __block CGFloat startingButtonX = [self startButtonX];

        [self.buttons enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            UIButton *button = obj;
            switch (self.alertButtonStyle) {
                case FUIAlertViewButtonLayoutVertical:
                    ...
                    break;
                case FUIAlertViewButtonLayoutHorizontal:
                    
                    [self setButton:obj atWidth:startingButtonX];
                    NSLog(@"%f",button.frame.size.width);
                    startingButtonX += (button.frame.size.width + 30 + self.buttonSpacing);
                    break;
...
- (void)setButton:(UIButton *)button atHeight:(CGFloat)height {
    ...
    CGFloat x = self.alertContentContainer.center.x;
    buttonFrame.origin = CGPointMake(0, height);
    buttonFrame.size.width = self.alertContentContainer.frame.size.width - 40;
    button.frame = buttonFrame;
    button.center  = CGPointMake(x - 15, button.center.y);
}

- (void)setButton:(UIButton *)button atWidth:(CGFloat)width {
    CGRect buttonFrame = button.frame;
    NSLog(@"%f",button.frame.size.height);
    buttonFrame.origin = CGPointMake(width, self.alertContainer.frame.size.height - button.frame.size.height - 20);
    buttonFrame.size.width = self.alertContentContainer.frame.size.width / 2 - self.buttonSpacing/2 - 30;
...
- (NSInteger)addButtonWithTitle:(NSString *)title {
    ...
    FUIButton *button = [[FUIButton alloc] initWithFrame:CGRectMake(0, 0, 0, self.alertButtonStyle == FUIAlertViewButtonLayoutHorizontal?30:44)];
